...
/* usbRxBuf can be in 2 bytes addressable range now. yanglq 2005-10-7*/
uchar	        usbRxBuf[2][USB_BUFSIZE];/* raw RX buffer: PID, 8 bytes data, 2 bytes CRC */
uchar		usbDeviceId;		/* assigned during enumeration, defaults to 0 */
unsigned char   *usbInputBuf;		/* ptr to raw buffer used for receiving */
unsigned char   *usbAppBuf;			/* ptr to raw buffer passed to app for processing */
...
volatile signed char usbTxLen = -1;	/* number of bytes to transmit with next IN token */
...
__flash static char usbDescrDevice[] = {	/* USB device descriptor */
...
__flash static char usbDescrConfig[] = {	/* USB configuration descriptor */
...
__flash static char usbDescrString0[] = {	/* language descriptor */
 ...
 */
#ifndef PRG_RDB
#define	PRG_RDB(addr)	__load_program_memory((unsigned char __flash *)addr)
#endif
...
void	usbInit(void)
{
	usbInputBuf = usbRxBuf[0];
	usbAppBuf = usbRxBuf[1];
